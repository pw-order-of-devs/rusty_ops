...
async fn basic_auth_positive_test() {
    let db = RunnableImage::from(Redis).start().await;
...
async fn db_connect(db: &ContainerAsync<Redis>) -> DbClient {
    let connection_string = &format!("redis://localhost:{}", db.get_host_port_ipv4(6379).await);
    let redis_client = RedisClient::from_string(connection_string).await;
    DbClient::Redis(redis_client)
}
