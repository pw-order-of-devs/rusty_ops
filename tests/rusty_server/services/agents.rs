...
async fn create_test() {
    ...
    let result = service::create(&db_client, RegisterAgent { id: "eb083ba6-0a61-4e01-a9a3-8471b8df2ee2".to_string() }).await;
    let _ = db.stop().await;
    assert!(result.is_ok());
}
...
async fn delete_by_id_test() {
    let db = RunnableImage::from(Redis)
        ...
        .expect("initializing test container failed");
    let db_client = db_connect(&db, "redis", 6379).await;
    let id = db_client.create("agents", &Agent { id: "uuid".to_string(), expiry: 0 }).await;
